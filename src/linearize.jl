...
function linearize(x,u,j,GravityInInertial)
    function MDiff(x::Vector{T}) where T<:Real
        ...
        return M, dM, M_inv, dM_inv, Ms, dMs, Ms_inv, dMs_inv
    end
    ...
    function accDiff(x::Vector{T}) where T<:Real
        dacc = zeros(T,(7*length(j),length(x)))
        for k=1:length(j)
            ct1 = 7*(k-1); ct2 = 14*k;
            # acceleration (ẍ)
            ...
            dacc2_1 = kron(permutedims(j[k].RB2.invJ*E*((Fc+Fu)[ct1+4:ct1+7])),I(size(E,1)))*fdJ_trE
            dacc2_2 = kron(permutedims((Fc+Fu)[ct1+4:ct1+7]),permutedims(E)*j[k].RB2.invJ)*fdJ_E
            dacc2_3 = permutedims(E)*j[k].RB2.invJ*E*(dFc[ct1+4:ct1+7,:] + dFu[ct1+4:ct1+7,:])
...
# @btime FiniteDifferences.jacobian(m, z->fxdot(z,u0,j,g),x0)[1];
