...
function ForceFree(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#,extF::extForces, GravityInInertial::Vector{Float64})::Vector{Float64}
...
function ForceConRev2In(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF2::extForces, GravityInInertial::Vector{Float64})::Vector{Float64}
...
function ForceConWeldAllIn(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF2::extForces, GravityInInertial::Vector{Float64})::Vector{Float64}
...
function ForceConSphIn(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF2::extForces, GravityInInertial::Vector{Float64})::Vector{Float64}
...
function ForceConSprIn(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF2::extForces, GravityInInertial::Vector{Float64})::Vector{Float64}
...
# For 2 rigid bodies
function ForceConRev(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF1::extForces, extF2::extForces, Fc1::Vector{Float64}, Fc2::Vector{Float64}, GravityInInertial::Vector{Float64})::Vector{Float64}
    # Constraint Force generated by revolute joint
    b1 = j.RB1
    b2 = j.RB2

    # Revolute Joint imposes 7 constraints between two rigid bodies
...
function ForceConRev2(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF1::extForces, extF2::extForces, Fc1::Vector{Float64}, Fc2::Vector{Float64},    GravityInInertial::Vector{Float64})::Vector{Float64}
    # Constraint Force generated by revolute joint
    b1 = j.RB1
    b2 = j.RB2

    # Revolute Joint2 has 6 constraints
    A = zeros(6,14); b = zeros(6);
    A[1:3,:] = TranslationConstraint(j)[1]; b[1:3] = TranslationConstraint(j)[2]
    A[4:5,:] = QuatNormConstraint(j)[1]; b[4:5] = QuatNormConstraint(j)[2]
...
function ForceConSph(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF1::extForces, extF2::extForces, Fc1::Vector{Float64}, Fc2::Vector{Float64}, GravityInInertial::Vector{Float64})::Vector{Float64}
    # Constraint Force generated by spherical joint
    b1 = j.RB1
    b2 = j.RB2
...
function ForceConSpr(j::Joint)::Tuple{Matrix{Float64}, Vector{Float64}}#, extF1::extForces, extF2::extForces, Fc1::Vector{Float64}, Fc2::Vector{Float64},     GravityInInertial::Vector{Float64})::Vector{Float64}
    # To compute Spring force and torque
    posSpr1 = j.RB1.x[1:3] + transpose(j.RB1.dcm)*j.pos1 # Inertial position of spring connection on first body
    posSpr2 = j.RB2.x[1:3] + transpose(j.RB2.dcm)*j.pos2 # Inertial position of spring connection on second body
    unitVec = (posSpr2 - posSpr1)/norm(posSpr2-posSpr1,2) # unit Vector in direction of spring (first body to second body)
...
function ConstraintForceTorque(M::Matrix{Float64},F::Vector{Float64},A::Matrix{Float64},b::Vector{Float64})::Vector{Float64}
    ...
    if !isreal(sqrt(M)) # insignificant imaginary values showing up
        ...
        Fc = sqrt(M)*pinv(A*M^(-0.5))*(b-A*accUnconstr)
    end
    return Fc
