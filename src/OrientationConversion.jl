...
function dcm2quat(C::Matrix{T})::Vector{T} where T <: Real
...
function quat2dcm(C::Matrix{T},q::Vector{T})::Matrix{T} where T <: Real
    β0 = q[1];
    β1 = q[2];
    β2 = q[3];
    β3 = q[4];
...
function quat2dcm(q::Vector{T})::Matrix{T} where T <: Real
...
function skewX(x::Vector{T})::Matrix{T} where T<: Real
    X = [    0 -x[3]  x[2]
          x[3]     0 -x[1]
         -x[2]  x[1]    0]
    return X
