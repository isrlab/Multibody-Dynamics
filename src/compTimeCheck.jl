...
# Testing ModRob case with random input values at every dt.
# Checking computation time of xdot = f(x,t)
function fCheck(j::Tuple{Vararg{Joint}}, uncF::Matrix{Float64}, cF::Matrix{Float64}, t::Float64)
    dx = Vector{Float64}(undef,14*10)
    dx[1:14] = zeros(14)
    for i=1:9
        dx[14*i+1:14*(i+1)] = rbDynQuat(j[i].RB2, uncF[:,j[i].RB2.bodyID], cF[:,j[i].RB2.bodyID])
    end
end

function TimeCheck(j)
    for i=1:112
        uncF = zeros(7,10); cF = zeros(7,10)
        for k=1:9
            j[k].RB2.x = rand(14)
            uncF[:,j[k].RB2.bodyID] = rand(7)
            cF[:,j[k].RB2.bodyID] = rand(7)
        end
        dy = fCheck(j, uncF, cF, Float64(i))
    end
end
@time TimeCheck(j)
